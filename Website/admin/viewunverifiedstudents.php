...
<?php
$raw_results = mysqli_query($conn, sprintf("CALL View_Unverified_Students_As_Administrator('%s')",$_SESSION['admin_username']));
    ...
    if (mysqli_num_rows($raw_results) < 1) {
        die("<div class=\"container\">
            <div class=\"alert alert-info\">
                <p><strong>All Done! </strong>The system has no unverified students for you to view!</p>
            </div>
            </div>");
    }

    display_results($raw_results, array( "First Name","Last Name","Age","gender", "Grade"), "Unverified Students");
}
?>
<?php
function display_results($results, $cols, $header_title)
{
    ...
    while ($result = mysqli_fetch_array($results)) {
        ...
        echo "<td><a href='verifystudent.php?ssn=" . $result['child_ssn'] .  "' class='btn btn-primary' role='button'>Verify Student!</a>";
