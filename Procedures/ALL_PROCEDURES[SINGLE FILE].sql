...
DELIMITER //
CREATE PROCEDURE View_Unverified_Teachers_As_Administrator(IN adminstrator_username VARCHAR(50))
  BEGIN
    DECLARE adminstrator_school_id INT;
    CALL GetAdministratorSchoolID(adminstrator_username, adminstrator_school_id);
    ...
    FROM Teachers
      INNER JOIN Employees ON username = Teachers.teacher_username
    WHERE school_id = adminstrator_school_id AND Employees.password IS NULL;
...
DELIMITER //
DROP PROCEDURE IF EXISTS View_Unverified_Students_As_Administrator;
CREATE PROCEDURE View_Unverified_Students_As_Administrator(IN adminstrator_username VARCHAR(50))
  BEGIN
    DECLARE adminstrator_school_id INT;
    CALL GetAdministratorSchoolID(adminstrator_username, adminstrator_school_id);
    ...
    THEN
      SELECT *
      FROM Students s
        INNER JOIN Children ON s.child_ssn = Children.ssn
      WHERE school_id = adminstrator_school_id AND s.password IS NULL ;
    END IF;
  END//
DELIMITER ;
...
DELIMITER //
DROP PROCEDURE IF EXISTS Assign_Teacher_To_Courses;
CREATE PROCEDURE Assign_Teacher_To_Courses(usrname VARCHAR(50), c_id INT, tchrusrname VARCHAR(50))
  ...
  IF (NOT EXISTS(SELECT *
                  FROM Courses_Taught_In_School_By_Teacher c
                  WHERE c.school_id=s_id AND c.course_code=c_id))
  THEN
    INSERT INTO Courses_Taught_In_School_By_Teacher (school_id, course_code, teacher_username)
    VALUES (s_id, c_id, tchrusrname);
  ELSE

        UPDATE Courses_Taught_In_School_By_Teacher
        SET teacher_username = tchrusrname
        WHERE school_id=s_id AND course_code=c_id;
  END IF;
  END //
DELIMITER ;

DELIMITER //
DROP PROCEDURE IF EXISTS View_Courses_as_Administrator;
CREATE PROCEDURE View_Courses_as_Administrator(usrname VARCHAR(50))
  BEGIN
    DECLARE s_id INT DEFAULT 0;
    ...
    SELECT c.*, es.school_id
    FROM Courses c , Courses_Offered_By_Levels co,Elementary_Schools es
    WHERE c.code=co.course_code AND co.level_id=es.level_id AND es.school_id=s_id
      Union
      SELECT c2.*, ms.school_id
      FROM Courses c2 , Courses_Offered_By_Levels co2,Middle_Schools ms
      WHERE c2.code=co2.course_code AND co2.level_id=ms.level_id AND ms.school_id=s_id
        Union
        SELECT c3.*, hs.school_id
        FROM Courses c3 , Courses_Offered_By_Levels co3,High_Schools hs
        WHERE c3.code=co3.course_code AND co3.level_id=hs.level_id AND hs.school_id=s_id
        ;
  END //
DELIMITER ;
...
# NEEDS : IsAnAdministrator
DELIMITER //
DROP PROCEDURE IF EXISTS Accept_Or_Reject_Child;
...
DELIMITER //
DROP PROCEDURE IF EXISTS View_Applied_And_Unverified_Children;
CREATE PROCEDURE View_Applied_And_Unverified_Children(adminstrator_username VARCHAR(50))
  BEGIN
    DECLARE adminstrator_school_id INT;
    CALL GetAdministratorSchoolID(adminstrator_username, adminstrator_school_id);

    SELECT *
    From Children_Applied_For_By_Parents_In_Schools s INNER JOIN children c ON s.child_ssn=c.ssn
    WHERE s.school_id = adminstrator_school_id AND status IS NULL ;

  END //
DELIMITER ;


DROP PROCEDURE IF EXISTS DisplaySchoolInformation;
DELIMITER //
CREATE PROCEDURE DisplaySchoolInformation(adminstrator_username VARCHAR(50))
  BEGIN
     DECLARE adminstrator_school_id INT;
    CALL GetAdministratorSchoolID(adminstrator_username, adminstrator_school_id);
    ...
    SELECT *
    FROM Schools
      WHERE Schools.id = adminstrator_school_id;
  END//
DELIMITER ;
...
DROP PROCEDURE IF EXISTS IsCourseTakenByStudent;
DELIMITER //
CREATE PROCEDURE IsCourseTakenByStudent(usrname VARCHAR(50), c_code INT)
  BEGIN
    ...
    WHERE COL.grade = q_grade AND C.code = c_code
    ORDER BY C.name;
  END//
DELIMITER ;


DROP PROCEDURE IF EXISTS IsAssignmentInCourse;
DELIMITER //
CREATE PROCEDURE IsAssignmentInCourse(ass_id INT, c_code INT)
  BEGIN
      SELECT * FROM Assignments
        WHERE id = ass_id and course_code = c_code;
  END//
DELIMITER ;
...
DROP PROCEDURE IF EXISTS DisplayAccountInformation;
DELIMITER //
CREATE PROCEDURE DisplayAccountInformation(db_ssn INT)
  BEGIN
    ...
    FROM Students
      ...
      WHERE Students.child_ssn = db_ssn;
  END//
DELIMITER ;
...
CREATE PROCEDURE ViewAssignments(usrname VARCHAR(50), c_code INT)
  BEGIN
    DECLARE db_ssn INT DEFAULT 0;
    ...
    WHERE CTS.student_ssn = db_ssn AND CTS.course_code = c_code;
...
DROP PROCEDURE IF EXISTS ViewAssignmentsGrades;
DELIMITER //
CREATE PROCEDURE ViewAssignmentsGrades(usrname VARCHAR(50), c_code INT)
  BEGIN
    DECLARE db_ssn INT DEFAULT 0;
    ...
    WHERE S.student_ssn = db_ssn AND A.course_code = c_code
...
DELIMITER //
DROP PROCEDURE IF EXISTS ViewActivities;
...
DROP PROCEDURE IF EXISTS ApplyForActivity;
DELIMITER //
...
DELIMITER //
CREATE PROCEDURE ViewClubs(usrname VARCHAR(50))
  BEGIN
    DECLARE sch_id INT DEFAULT 0;
    ...
    SELECT * FROM Clubs WHERE high_school_id = sch_id;
  END //
DELIMITER ;
...
DROP PROCEDURE IF EXISTS ViewTeachers;
DELIMITER //
...
DROP PROCEDURE IF EXISTS ViewSchools;
DELIMITER //
CREATE PROCEDURE ViewSchools(usrname VARCHAR(50))
  BEGIN
    ...
    FROM Parents P INNER JOIN Children_Applied_For_By_Parents_In_Schools CAPS
      ...
      INNER JOIN Children ON CAPS.child_ssn = Children.ssn
    WHERE CAPS.parent_username = usrname AND CAPS.status = 1 AND
      NOT EXISTS (SELECT * FROM Students WHERE Students.child_ssn = Children.ssn)
...
DROP PROCEDURE IF EXISTS ViewSchoolsList;
DELIMITER //
...
DROP PROCEDURE IF EXISTS ViewMyReviews;
DELIMITER //
CREATE PROCEDURE ViewMyReviews(usrname VARCHAR(50))
  BEGIN
    SELECT *
    FROM Parents_Review_Schools INNER JOIN Schools ON Parents_Review_Schools.school_id = Schools.id
    WHERE parent_username = usrname;
  END//
DELIMITER ;

CALL ViewMyReviews('kojak');
...
DELIMITER //
DROP PROCEDURE IF EXISTS ViewRating;
...
DROP PROCEDURE IF EXISTS ViewRatingsForAllTeachers;
DELIMITER //
CREATE PROCEDURE ViewRatingsForAllTeachers()
  BEGIN
...
CALL ViewRatingsForAllTeachers();
...
# the school id input in the procedure comes from php
DELIMITER //
CREATE PROCEDURE Make_Schools_Middle(s_id INT)
  BEGIN

    INSERT INTO Middle_Schools (school_id, level_id)
    VALUES (s_id, 2);
...
# the school id input in the procedure comes from php
DELIMITER //
CREATE PROCEDURE Make_Schools_High(s_id INT)
  BEGIN

    INSERT INTO High_Schools (school_id, level_id)
    VALUES (s_id, 3);
...
DROP PROCEDURE IF EXISTS View_AllAvailable_Schools;
DELIMITER //
...
DELIMITER //
CREATE PROCEDURE View_Specific_School_Announcements(IN curr_school_id INT)
  BEGIN
    SELECT *
    FROM Announcements
      JOIN Employees
        ON Announcements.administrator_username = Employees.username
    WHERE school_id = curr_school_id;
  END//
DELIMITER ;
...
# 4 -
DELIMITER //
CREATE PROCEDURE TeacherViewAssignment(usrname VARCHAR(50))
  BEGIN
    SELECT
    content, posting_date, due_date, id, course_code
    FROM Assignments
    WHERE teacher_username = usrname;
  END//
DELIMITER ;

# 4 - View the studentsâ solutions for the assignments I posted ordered by studentsâ ids.
DELIMITER //
...
use M3_School;
# 4 -
DROP PROCEDURE IF EXISTS TeacherViewAssignment;
DELIMITER //
CREATE PROCEDURE TeacherViewAssignment(usrname VARCHAR(50))
  BEGIN
    SELECT
...
# 4 - View the studentsâ solutions for the assignments I posted ordered by studentsâ ids.
DROP PROCEDURE IF EXISTS TeacherViewAssignmentSolution;
DELIMITER //
CREATE PROCEDURE TeacherViewAssignmentSolution(usrname VARCHAR(50))
  BEGIN
    SELECT
...
DROP PROCEDURE IF EXISTS Teacher_View_Questions;
DELIMITER //
CREATE PROCEDURE Teacher_View_Questions(usrname VARCHAR(50))
  BEGIN
    ...
    FROM Courses_Questions_From_Students CQS INNER JOIN Questions Q ON Q.id = CQS.question_id
      INNER JOIN Courses_Taught_In_School_By_Teacher CTBT ON CQS.course_code = CTBT.course_code
      INNER JOIN Children C ON CQS.student_ssn = C.ssn
...
DELIMITER //
DROP PROCEDURE IF EXISTS Teacher_list_Students;
